...
import { useHotelServiceRequests, useHotelServiceRequestDetails } from '@/hooks/hotel/useHotelServiceRequests';
...
export default function ServiceRequestDrawer({ open, onClose, requestId }: ServiceRequestDrawerProps) {
  const { data: response, isLoading } = useHotelServiceRequestDetails(requestId || '');
  const { updateNotes, isUpdating } = useHotelServiceRequests();
  ...
  const handleUpdateNotes = () => {
    if (!requestId) return;
    updateNotes({ id: requestId, data: { adminNotes: notes } });
  };

  const handleCloseRequest = () => {
    if (!requestId) return;
    updateNotes({ id: requestId, data: { adminNotes: notes, status: 'CLOSED' } });
  };
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <Descriptions bordered column={1} size="small">
              ...
              <Descriptions.Item label="Registered User">{request.user?.name || request.user?.email || 'Guest'}</Descriptions.Item>
