...
export default function LoginPage() {
  ...
  const onFinish = async (values: any) => {
    try {
      setLoading(true);
      const response: any = await api.post('/auth/hotel/login', values);
      
      if (response.success && response.data?.token) {
        localStorage.setItem('hotel_token', response.data.token);
        localStorage.setItem('hotel_info', JSON.stringify(response.data.user));
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <Card className="w-full max-w-md shadow-lg rounded-xl">
        <div className="text-center mb-8">
          <Title level={2} className="m-0">Citylink Hotel</Title>
